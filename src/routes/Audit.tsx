  ...
  const handleRestore = useCallback(
    async (entry: AuditEntry) => {
      ...
      try {
        ...
        ctx.pushToast("success", `Záznam ${entry.issue_key ?? ""} byl obnoven v Jira.`);
        reload();
      } catch (e) {
        ctx.pushToast(
          "error",
          typeof e === "string" ? e : "Obnovu se nepodařilo dokončit",
        );
      } finally {
        setBusyId(null);
      }
  ...
  const handleRevert = useCallback(
    async (entry: AuditEntry) => {
      ...
      try {
        ...
        ctx.pushToast("success", `Změna na ${entry.issue_key ?? ""} byla vrácena.`);
        reload();
      } catch (e) {
        ctx.pushToast(
          "error",
          typeof e === "string" ? e : "Vrácení se nepodařilo dokončit",
        );
      } finally {
        setBusyId(null);
      }
  ...
  const handleRetry = useCallback(
    async (entry: AuditEntry) => {
      ...
      try {
        ...
        ctx.pushToast("success", "Akce byla úspěšně opakována.");
        reload();
      } catch (e) {
        ctx.pushToast(
          "error",
          typeof e === "string" ? e : "Opakování se nepodařilo",
        );
      } finally {
        setBusyId(null);
      }
...
export function groupByDay(
  ...
  for (const e of entries) {
    ...
    const k = dayKey(dt);
    let label: string;
    if (k === todayKey) label = "Dnes";
    else if (k === yesterdayKey) label = "Včera";
