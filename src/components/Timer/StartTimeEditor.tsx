...
export function StartTimeEditor({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="text-xs font-medium text-[var(--text-secondary)]">{t("worklog.startEditor.label")}</div>
      <div className="flex items-center gap-2 flex-wrap">
        <IconButton
          aria-label={t("worklog.startEditor.minus5")}
        ...
        <IconButton
          aria-label={t("worklog.startEditor.minus1")}
        ...
        <input
          type="time"
          aria-label={t("worklog.startEditor.timeInput")}
          value={timeValue}
        ...
        <IconButton
          aria-label={t("worklog.startEditor.plus1")}
          onClick={() => onChange(clampedDelta(60_000))}
        >
        ...
        <IconButton
          aria-label={t("worklog.startEditor.plus5")}
          onClick={() => onChange(clampedDelta(5 * 60_000))}
        >
          <Plus className="w-3.5 h-3.5" aria-hidden />
          <span aria-hidden className="text-[10px] ml-0.5">
        ...
        <Button
          ...
          aria-label={t("worklog.startEditor.resetNow")}
        >
          {t("worklog.startEditor.now")}
        </Button>
      </div>
    </div>
  );
